const faceDataUrl = 'http://localhost:3000/faces/data';
const updateUrl = 'http://localhost:5000/faces/update';
...
function markForDeletion(id) {
    ...
    if (!facesToDelete.includes(id)) {
        ...
        row.classList.add('deleting'); // Add class for greying out
    } else {
        facesToDelete = facesToDelete.filter(faceId => faceId !== id);
        row.classList.remove('deleting'); // Remove class to restore
    }
...
function updateAllFaces() {
    ...
    // const updateData = {
    //     faces: facesData,
    //     deleteIds: facesToDelete, // Include the deletion IDs
    // };
